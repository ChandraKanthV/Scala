...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (w.toLowerCase().groupBy (c => c) toList) map
      (pair =>{val (c,l) = pair; (c, l.length)} ) sorted
  }
   ...
   */
  def combinationsHelper(pair:(Char, Int), zero:List[Occurrences] ) :List[Occurrences] = pair match {
    case (char, occ) =>  zero ++ ( for { zeroPair <- zero; i <- 1 to occ } yield (char, i) :: zeroPair )
  }
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences] (Nil)) ( (a,z) => (combinationsHelper(a,z) )   )
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (y foldLeft SortedMap [Char, Int]() ++ x) ((x, y1) => subtractHelper(x, y1)) toList
